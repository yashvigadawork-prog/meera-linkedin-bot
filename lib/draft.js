...
function buildUserPrompt(transcript, angle, newsItem) {
...
async function draftWithGemini(transcript, angle, newsItem) {
  const data = await generateContent(
    config.geminiDraftModel,
    {
      systemInstruction: { parts: [{ text: SYSTEM_PROMPT }] },
      contents: [{ role: 'user', parts: [{ text: buildUserPrompt(transcript, angle, newsItem) }] }],
...
async function draftWithClaude(transcript, angle, newsItem) {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: 'user', content: buildUserPrompt(transcript, angle, newsItem) }],
    }),
...
// caller appends the verify-flag block only when true.
export async function generateDraft(transcript, angle) {
  ...
  const rawText =
    config.draftProvider === 'claude'
      ? await draftWithClaude(transcript, angle, newsItem)
      : await draftWithGemini(transcript, angle, newsItem);
