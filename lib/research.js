...
async function extractSearchQueries(fragment, angle) {
  const prompt =
    `${BRAND_CONTEXT}\n\n` +
    'Given this raw note from Meera, output 1 to 2 short, effective news ' +
    'search queries (each under 8 words) that would find real supporting ' +
    'industry news for a LinkedIn post built around this note. Output ONLY ' +
    'the queries, one per line, no numbering, no quotes, no explanation. ' +
    'If the note genuinely has no researchable angle (pure opinion with no ' +
    'topic to look up), output nothing.\n\n' +
    `Note:\n"""\n${fragment}\n"""` +
    (angle ? `\n\nSupplied angle:\n"""\n${angle}\n"""` : '');
...
// Runs the whole research step. Never throws - a failure here (quota,
// network) should degrade to "no research", not break drafting, since
// research is an enhancement on top of Meera's own fragment.
